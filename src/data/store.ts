...
export function startLiveStream(input: {
  ...
  const newStream: LiveStream = {
    ...
    description: input.description?.trim() || "Live community broadcast and Q&A session with KR8 Digitals.",
    hostId: input.host.id,
    hostName,
    hostAvatar: input.host.avatar || (input.host.type === "founder" ? "/founder_timfire.jpg" : undefined),
    isLive: true,
    startedAt: Date.now(),
    viewerCount: 42, // initial live audience
    peakViewers: 42,
    quality: input.quality || "1080p60",
    videoUrl: "/videos/testimonial_grant_gideon.mp4",
    posterUrl: "/founder_timfire_wide.jpg",
    pinnedNotice: "Welcome to the official KR8 Live Stream! Type in chat to ask questions.",
    promotedModerators: [],
    promotedSpeakers: [],
  };
...
export function endActiveLiveStream(): StreamReplay | null {
  ...
  const replay: StreamReplay = {
    ...
    videoUrl: stream.videoUrl || "/videos/testimonial_grant_gideon.mp4",
    messagesCount: messages.length,
    requiresAccount: true, // Gate for re-streaming or watching replays
  };
